...
// USE THIS FOR YOUR CACHE STATISTICS AND BITS
                  ...
                  cache_org_t cache_org)
{
  ...
  if (cache_mapping == dm) {
    if (cache_org == uc) {
      cache_bits->index = countBits(index_size);
    } else { /* split cache */
      cache_bits->index = countBits(index_size) / 2;
    }
...
void main(int argc, char** argv) {
  ...
  /* Setup cache itself, define tag, index, and offset bit values */
